...
const llvm::StringSet<> ErrnoTestingCheck::ErrnoSettingFunctions = {
    ...
    "cosh",    "coshf",   "coshl",   "exp",     "expf",    "expl",
    "exp2",    "exp2f",   "exp2l",   "expm1",   "expm1f",  "expm1l",
    "fdim",    "fdimf",   "fdiml",   "fma",     "fmaf",    "fmal",
    ...
    "lround",  "lroundf", "lroundl", "nextafter", "nextafterf", "nextafterl",
    "nexttoward", "nexttowardf", "nexttowardl",
    "pow",     "powf",    "powl",    "remainder", "remainderf", "remainderl",
    "remquo",  "remquof", "remquol", "rint",    "rintf",   "rintl",
    "scalbln", "scalblnf", "scalblnl", "scalbn", "scalbnf", "scalbnl",
    "sinh",    "sinhf",   "sinhl",   "sqrt",    "sqrtf",   "sqrtl",
    "tgamma",  "tgammaf", "tgammal", "y0",      "y1",      "yn",
    // <stdlib.h> functions
    "strtol",  "strtoll", "strtoul", "strtoull", "strtof",  "strtod",
    "strtold", "strtoimax", "strtoumax", "wcstol", "wcstoll", "wcstoul",
    "wcstoull", "wcstof", "wcstod",  "wcstold", "wcstoimax", "wcstoumax",
    // <stdio.h> functions
    "fopen",   "freopen", "fclose",  "fflush",  "fread",   "fwrite",
    "fgetc",   "fgets",   "fputc",   "fputs",   "getc",    "putc",
    "ungetc",  "fseek",   "ftell",   "fsetpos", "fgetpos", "rewind",
    "fprintf", "fscanf",  "printf",  "scanf",   "snprintf", "sprintf",
    "sscanf",  "vfprintf", "vfscanf", "vprintf", "vscanf", "vsnprintf",
    "vsprintf", "vsscanf", "fgetwc", "fgetws",  "fputwc",  "fputws",
    "getwc",   "putwc",   "ungetwc", "fwprintf", "fwscanf", "wprintf",
    "wscanf",  "swprintf", "swscanf", "vfwprintf", "vfwscanf", "vwprintf",
    "vwscanf", "vswprintf", "vswscanf", "remove", "rename",  "tmpfile",
    "tmpnam",
    // <string.h> functions
    "strerror",
    // <wchar.h> functions
    ...
    "fstat",   "lstat",   "mkdir",   "rmdir",   "unlink",  "link",
    "symlink", "readlink", "chmod",  "chown",   "access",  "dup",
    "dup2",    "pipe",    "socket",  "bind",    "listen",  "accept",
    "connect", "send",    "recv",    "sendto",  "recvfrom", "setsockopt",
    "getsockopt", "getaddrinfo", "getnameinfo", "pthread_create",
    "pthread_join", "pthread_mutex_init", "pthread_mutex_lock",
    "pthread_mutex_unlock", "pthread_cond_init", "pthread_cond_wait",
    "pthread_cond_signal", "sem_open", "sem_close", "sem_wait", "sem_post",
    "mmap",    "munmap",  "mprotect", "msync",   "fork",    "exec",
    "execve",  "execvp",  "wait",    "waitpid", "kill",    "raise",
    "alarm",   "sleep",   "usleep",  "nanosleep", "getcwd", "chdir",
    "opendir", "readdir", "closedir", "getenv", "setenv",  "unsetenv",
    "system",  "popen",   "pclose",  "dlopen",  "dlsym",   "dlclose",
    // Memory allocation
...
void ErrnoTestingCheck::registerMatchers(MatchFinder *Finder) {
  ...
  // errno is typically a macro that expands to something like (*__errno_location())
  // We look for DeclRefExpr to a variable named "errno" or calls to __errno_location
  Finder->addMatcher(
      binaryOperator(
          ...
          anyOf(hasLHS(ignoringParenImpCasts(
                ...
                hasLHS(ignoringParenImpCasts(unaryOperator(
                    hasOperatorName("*"),
                    hasUnaryOperand(callExpr(callee(functionDecl(
                        anyOf(hasName("__errno_location"),
                              hasName("__errno"))))))))),
