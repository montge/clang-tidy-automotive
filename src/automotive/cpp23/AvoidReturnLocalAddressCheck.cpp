...
void AvoidReturnLocalAddressCheck::registerMatchers(MatchFinder *Finder) {
      ...
      returnStmt(
          hasReturnValue(unaryOperator(
              ...
              hasUnaryOperand(declRefExpr(to(varDecl(
                  ...
                  unless(hasStaticStorageDuration())).bind("var")))))))
          .bind("return_addr"),
      this);
  ...
  Finder->addMatcher(
      returnStmt(hasReturnValue(
          implicitCastExpr(hasSourceExpression(declRefExpr(to(varDecl(
              hasAutomaticStorageDuration(),
              unless(hasStaticStorageDuration())).bind("ref_var")))))))
          .bind("return_ref"),
      this);
  ...
  Finder->addMatcher(
      cxxThrowExpr(
          has(unaryOperator(
              ...
              hasUnaryOperand(declRefExpr(to(varDecl(
                  hasAutomaticStorageDuration(),
                  unless(hasStaticStorageDuration())).bind("throw_var")))))))
          .bind("throw_addr"),
      this);
}
...
void AvoidReturnLocalAddressCheck::check(
  ...
  if (const auto *ReturnAddr = Result.Nodes.getNodeAs<ReturnStmt>("return_addr")) {
  ...
  if (const auto *ReturnRef = Result.Nodes.getNodeAs<ReturnStmt>("return_ref")) {
    if (const auto *Var = Result.Nodes.getNodeAs<VarDecl>("ref_var")) {
      // Only flag if the function returns a reference type
      if (const auto *Func = Result.Context->getParents(*ReturnRef)[0]
              .get<FunctionDecl>()) {
  ...
  if (const auto *ThrowAddr = Result.Nodes.getNodeAs<CXXThrowExpr>("throw_addr")) {
