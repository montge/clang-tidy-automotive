...
void test_bool_arithmetic_violation(bool flag) {
  // CHECK-MESSAGES: :[[@LINE+1]]:11: warning: inappropriate essential type
  int x = flag + 1;
  (void)x;
}
...
void test_char_arithmetic_violation(char c) {
  // CHECK-MESSAGES: :[[@LINE+1]]:12: warning: inappropriate essential type
  char d = c + 1;
  (void)d;
}

void test_mixed_signed_unsigned_violation(int s, unsigned int u) {
  // CHECK-MESSAGES: :[[@LINE+1]]:11: warning: inappropriate essential type
  int r = s + u;
  (void)r;
}
