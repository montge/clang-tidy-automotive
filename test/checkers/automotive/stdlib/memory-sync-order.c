...
void test_violations(void) {
    ...
    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: atomic operation shall use memory_order_seq_cst [automotive-c25-req-21.25]
    ...
    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: atomic operation shall use memory_order_seq_cst for success ordering [automotive-c25-req-21.25]
    atomic_compare_exchange_strong_explicit(&global_counter, &expected, desired,
                                            memory_order_acquire, memory_order_relaxed);

    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: atomic operation shall use memory_order_seq_cst for failure ordering [automotive-c25-req-21.25]
    atomic_compare_exchange_weak_explicit(&global_counter, &expected, desired,
                                          memory_order_seq_cst, memory_order_relaxed);

    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: atomic fence operation shall use memory_order_seq_cst [automotive-c25-req-21.25]
