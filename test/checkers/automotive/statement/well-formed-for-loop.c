...
void test_missing_init(int i) {
    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: for loop is missing initialization; a well-formed for loop should initialize a loop counter [automotive-c25-req-14.2]
    for (; i < 10; i++) {
...
void test_missing_condition(void) {
    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: for loop is missing condition; a well-formed for loop should test the loop counter [automotive-c25-req-14.2]
    for (int i = 0; ; i++) {
        if (i >= 10) break;
    }
}
...
void test_missing_increment(void) {
    // CHECK-MESSAGES: :[[@LINE+1]]:5: warning: for loop is missing increment expression; a well-formed for loop should modify the loop counter in the increment expression [automotive-c25-req-14.2]
    for (int i = 0; i < 10; ) {
...
void test_counter_not_in_condition(int limit) {
    // CHECK-MESSAGES: :[[@LINE+1]]:27: warning: for loop condition does not reference the loop counter [automotive-c25-req-14.2]
...
void test_counter_not_modified_in_increment(int j) {
    // CHECK-MESSAGES: :[[@LINE+1]]:30: warning: for loop increment expression does not modify the loop counter [automotive-c25-req-14.2]
    for (int i = 0; i < 10; j++) {
    }
}
...
void test_counter_modified_in_body_increment(void) {
    for (int i = 0; i < 10; i++) {
        // CHECK-MESSAGES: :[[@LINE+1]]:10: warning: loop counter modified in for loop body; the loop counter should only be modified in the increment expression [automotive-c25-req-14.2]
        i++;
    }
}
...
void test_counter_modified_in_body_assign(void) {
    for (int i = 0; i < 10; i++) {
        // CHECK-MESSAGES: :[[@LINE+1]]:11: warning: loop counter modified in for loop body; the loop counter should only be modified in the increment expression [automotive-c25-req-14.2]
        i = 5;
    }
}
