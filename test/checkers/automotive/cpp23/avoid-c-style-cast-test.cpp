...
void test_c_style_cast_int_violation() {
  double d = 3.14;
  // CHECK-MESSAGES: :[[@LINE+1]]:11: warning: C-style cast shall not be used
  int i = (int)d;
  (void)i;
}
...
void test_c_style_cast_pointer_violation() {
  int x = 42;
  // CHECK-MESSAGES: :[[@LINE+1]]:15: warning: C-style cast shall not be used
  void *ptr = (void *)&x;
  (void)ptr;
}
...
void test_c_style_cast_const_violation() {
  ...
  // CHECK-MESSAGES: :[[@LINE+1]]:13: warning: C-style cast shall not be used
  int *p = (int *)cp;
  (void)p;
}
